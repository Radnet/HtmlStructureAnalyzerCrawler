...
namespace Classificator
{
	class Program
	{
		...
		private static void Execute (FlexibleOptions options)
		{
            ...
            // Get Domain
            string domains = ConsoleUtils.ProgramOptions.Get ("DOMAINS");
            ...
            foreach (string domain in domainList)
            {
                ...
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"ReportResult"));

                // Show results and write on file
                using (var writer = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"ReportResult\report.txt"), true, System.Text.Encoding.GetEncoding("ISO-8859-1"), 4 * 1024))
                {
                    writer.WriteLine();
                    writer.WriteLine();
                    writer.WriteLine();
                    ...
                    writer.WriteLine("news_distance = " + news_distance);
                    writer.WriteLine("ecos_distance = " + ecos_distance);
                    writer.WriteLine("govs_distance = " + govs_distance);
                    writer.WriteLine();
                    ...
                    else if (ecos_distance < news_distance)
                    {
                        ...
                        if (ecos_distance < govs_distance)
                        {
                            ...
                            writer.WriteLine("It's a ECCOMERCE !");
                        }
                        // Its a GOV
                        ...
                        else
                        {
                            ...
                            writer.WriteLine("It's a TIE between ECCOMERCE and GOVERNAMENTAL !");
                        }
                    }
                    ...
                    else
                    {
                        ...
                        writer.WriteLine("It's a TIE between NEWS and ECCOMERCE !");
                    }
                }
                //###########################
            }
